...
void UKF::UpdateLidar(MeasurementPackage meas_package) {
  ...
  */
  ...
  //create noise cov matrix and add to S(predicted measurement cov)
  ...
  cout << "NIS Laser at time-> " << counter++ << " : " << NIS_laser_ << endl;
}
...
void UKF::UpdateRadar(MeasurementPackage meas_package) {
  ...
  */
  ...
  for (int i = 0; i < 2 * n_aug_ + 1; i++) {
	...
	Zsig(2,i) = (px*cos(yaw)*v+py*sin(yaw)*v) / Zsig(0,i); //rodot
  }
  ...
  cout << "NIS Radar at time-> " << counter++ << " : " << NIS_radar_ << endl;
}
